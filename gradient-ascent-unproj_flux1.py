...
def parse_arguments():
    ...
    parser.add_argument('--model_name', default='ViT-L/14', help="CLIP model name or path")
...
def generate_target_text_embeddings(img_path, model, lats, optimizer, scheduler, training_iterations, checkin_step, many_tokens, prompt, nom, augment, tok, bests, tokens_to, embeds_to, use_best):
    ...
    if use_best:
        ...
        device = best_text_embeddings.device
        best_text_modded_embeddings = copy.deepcopy(best_text_embeddings).to(device).float()
        # Undo the projection (unproject embeddings)
        if hasattr(model, 'text_projection'):  # Check if the model has a projection layer
            rank = torch.linalg.matrix_rank(model.text_projection.detach().to(device))
            print(f"Rank of text_projection: {rank}")

            # Move text_projection to the correct device
            text_projection_matrix = model.text_projection.detach().to(device).float()

            # Unproject using pseudo-inverse (pinv)
            try:
                text_projection_pinv = torch.linalg.pinv(text_projection_matrix)  # Compute pseudo-inverse
                unprojected_pinv = best_text_modded_embeddings @ text_projection_pinv.T

                # Normalize using ln_final (simulate CLIP's behavior)
                ln_final_norm_pinv = unprojected_pinv.norm(p=2, dim=-1, keepdim=True)
                ln_final_weight_norm = model.ln_final.weight.norm(p=2).to(device)  # Normalize using ln_final.weight
                unprojected_pinv = unprojected_pinv / ln_final_norm_pinv * ln_final_weight_norm

                # Add bias if present
                if model.ln_final.bias is not None:
                    ln_final_bias_adjustment = model.ln_final.bias.to(device)
                    unprojected_pinv += ln_final_bias_adjustment

                # Save the pinv-based embeddings
                torch.save(unprojected_pinv, f"{embeds_to}/unproj_pinv_{img_name}.pt")
            except Exception as e:
                print(f"Error during pseudo-inverse calculation: {e}")

            # Unproject using true inverse (inv)
            try:
                text_projection_inv = torch.inverse(text_projection_matrix)  # Compute true inverse
                unprojected_inv = best_text_modded_embeddings @ text_projection_inv.T

                # Normalize using ln_final (simulate CLIP's behavior)
                ln_final_norm_inv = unprojected_inv.norm(p=2, dim=-1, keepdim=True)
                unprojected_inv = unprojected_inv / ln_final_norm_inv * ln_final_weight_norm

                # Add bias if present
                if model.ln_final.bias is not None:
                    unprojected_inv += ln_final_bias_adjustment  # Reuse bias adjustment from above

                # Save the inv-based embeddings
                torch.save(unprojected_inv, f"{embeds_to}/unproj_inv_{img_name}.pt")
            except Exception as e:
                print(f"Error during true inverse calculation: {e}")
        else:
            print("Model does not have text_projection. Using original embeddings.")
            unprojected_pooled_embedding = best_text_modded_embeddings
            torch.save(unprojected_pooled_embedding, f"{embeds_to}/{img_name}.pt")
        ...
        expected_norm = model.logit_scale.exp()
        print(f"Learned scaling factor (expected norm): {expected_norm}")
        rescaled_embeddings = target_text_embedding / target_text_embedding.norm(p=2, dim=-1, keepdim=True) * expected_norm
        torch.save(target_text_embedding, f"{embeds_to}/{img_name}.pt")
        torch.save(rescaled_embeddings, f"{embeds_to}/logitscale_{img_name}.pt")
        print(Fore.MAGENTA + Style.BRIGHT + f"\nBest text embedding saved to '{embeds_to}'.\nTokens (CLIP 'opinion') saved to '{tokens_to}'.\n" + Fore.RESET)
